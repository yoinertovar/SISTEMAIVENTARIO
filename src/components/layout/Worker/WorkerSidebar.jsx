/**
 * WorkerSidebar - Barra Lateral de Navegación del Trabajador
 * 
 * Sidebar espectacular con diseño moderno y tema verde-teal que incluye:
 * - Menú de navegación con 7 opciones específicas para trabajador
 * - Logo corporativo grande y centrado
 * - Efectos visuales de hover y activo con gradiente verde-teal
 * - Animaciones de brillo y transición
 * - Responsive con overlay en móvil
 * - Scrollbar personalizado con gradiente emerald-teal
 * - Footer con branding
 * 
 * @component
...
import { Home, Package, CreditCard, FileText, DollarSign, X, RotateCcw, Users } from 'lucide-react';

const WorkerSidebar = ({ isMobileOpen, setIsMobileOpen }) => {
  const navigate = useNavigate();
  const location = useLocation();

  /**
   * Configuración del menú de navegación del trabajador
   * 7 opciones específicas para las operaciones diarias
   */
  const menuItems = [
    { id: 'dashboard', label: 'INICIO', icon: Home, path: '/worker/dashboard' },
    { id: 'facturas', label: 'FACTURAS', icon: FileText, path: '/worker/facturas' },
    { id: 'inventario', label: 'INVENTARIO', icon: Package, path: '/worker/inventario' },
    { id: 'credito', label: 'CREDITO', icon: CreditCard, path: '/worker/credito' },
    { id: 'gastos', label: 'GASTOS', icon: DollarSign, path: '/worker/gastos' },
    { id: 'devoluciones', label: 'DEVOLUCIONES', icon: RotateCcw, path: '/worker/devoluciones' },
    { id: 'clientes', label: 'CLIENTES', icon: Users, path: '/worker/clientes' },
  ];
  ...
  return (
    <>
      ...
      {/* Sidebar principal con gradiente oscuro profesional y tema verde-teal */}
      <aside className={`
      ...
      `}>
        {/* Sección del Logo - Diseño espectacular con efectos de brillo verde-teal */}
        <div className="relative px-4 py-6 bg-gradient-to-br from-slate-700 to-slate-800 border-b border-slate-600 overflow-hidden">
          {/* Efectos de brillo decorativos en la esquina con tema verde-teal */}
        ...
        {/* Menú de navegación con efectos profesionales y tema verde-teal */}
            ...
            {menuItems.map(item => {
              ...
              return (
                <button
                  ...
                  `}
                >
                  {/* Efecto de brillo deslizante en hover con tema verde (solo para items no activos) */}
                  {!active && (
        ...
        {/* Footer con branding y gradiente espectacular verde-teal */}
      ...
      {/* Estilos personalizados para scrollbar con gradiente verde-teal */}
      <style jsx>{`
